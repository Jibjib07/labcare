...
try {
    ...
    if ($current_status !== 'Condemned') {
        $status_fields = [
            ...
            $_POST['mic_status'] ?? '',
            $_POST['hdmi_status'] ?? '',
            $_POST['headphone_status'] ?? '',
            $_POST['display_status'] ?? '',
            $_POST['inline_status'] ?? '',
            $_POST['ethernet_status'] ?? '',
            $_POST['disk_health'] ?? '',
            $_POST['power_health'] ?? '',
            $_POST['monitor_status'] ?? '',
        ...
        if ($com_age >= 5) {
            $overall_status = 'For Condemn';
        } elseif (in_array('For Repair', $status_fields)) {
            $overall_status = 'For Repair';
        }

        $stmt_main = $conn->prepare("UPDATE units SET set_status=? WHERE set_id=?");
        $stmt_main->bind_param("ss", $overall_status, $set_id);
        $stmt_main->execute();
        $stmt_main->close();
    }
