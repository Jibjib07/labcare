<?php
session_start(); // CRITICAL: Added session_start() so we can grab the user's name!
...
try {
    ...
    // --- NEW: FETCH FULL LAB NAME FOR THE LOGS ---
    $lab_full_name = "Unknown Lab ($lab_room)";
    $lab_stmt = $conn->prepare("SELECT lab_name FROM laboratories WHERE lab_id = ?");
    $lab_stmt->bind_param("i", $lab_id);
    $lab_stmt->execute();
    $lab_res = $lab_stmt->get_result();
    if ($lab_row = $lab_res->fetch_assoc()) {
        $lab_full_name = $lab_row['lab_name'] . " ($lab_room )";
    }
    $lab_stmt->close();
    // ---------------------------------------------
    ...
    foreach ($unit_tags as $tag) {
        ...
        if ($stmt->execute()) {
            ...
            $affected = 'Entire Set';
            $action = 'Add';
            $remarks = "Computer Set added with Set Tag - PC-" . str_pad($tag, 2, "0", STR_PAD_LEFT) . " in " . $lab_full_name;
            $status = 'Added';

            $history_stmt = $conn->prepare("INSERT INTO unit_history (set_id, lab_id, report_date, report_actor, report_affected, report_action, report_remarks, report_status) VALUES (?, ?, NOW(), ?, ?, ?, ?, ?)");
            $history_stmt->bind_param("sssssss", $new_set_id, $lab_id, $actor, $affected, $action, $remarks, $status);
